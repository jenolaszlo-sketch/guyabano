...
public sealed class SqliteCrossStoreOperationStore : ICrossStoreOperationStore
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (request.WorkflowRunId == Guid.Empty)
            throw new ArgumentException("A non-empty workflow run ID is required.", nameof(request));
        ...
        await using (var insert = connection.CreateCommand())
        {
            ...
            insert.CommandText = """
                INSERT INTO cross_store_operations(
                    operation_id, session_id, workflow_run_id, kind, idempotency_key,
                    state, created_at, updated_at, version, reconciliation_reason)
                VALUES($id, $sessionId, $runId, $kind, $key, $state, $created, $updated, 1, NULL);
                """;
            insert.Parameters.AddWithValue("$id", id.ToString());
            insert.Parameters.AddWithValue("$sessionId", request.SessionId.ToString());
            insert.Parameters.AddWithValue("$runId", request.WorkflowRunId.ToString("D"));
            ...
            {
                throw new InvalidOperationException(
                    $"Operation '{id}' conflicts with an existing operation or active workflow operation.",
                    exception);
            }
        }
    ...
    private async Task<SqliteConnection> OpenAsync(CancellationToken cancellationToken)
    {
        ...
        command.CommandText = $$"""
            ...
            CREATE UNIQUE INDEX IF NOT EXISTS ux_cross_store_active_workflow
                ON cross_store_operations(workflow_run_id)
        ...
        CancellationToken cancellationToken)
    {
        ...
        command.CommandText = """
            SELECT session_id, workflow_run_id, kind, idempotency_key, state,
        ...
        if (!await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
            return null;
        var sessionId = GuyabanoSessionId.Parse(reader.GetString(0));
        var workflowRunId = Guid.Parse(reader.GetString(1));
        var kind = reader.GetString(2);
        var key = reader.GetString(3);
        var state = (CrossStoreOperationState)reader.GetInt32(4);
        var createdAt = Parse(reader.GetString(5));
        var updatedAt = Parse(reader.GetString(6));
        var version = reader.GetInt64(7);
        var reason = reader.IsDBNull(8) ? null : reader.GetString(8);
