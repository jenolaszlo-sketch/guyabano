    ...
    TimeProvider timeProvider)
{
        ...
        CrossStoreOperation operation)
    {
        ...
        var action = operation.State switch
        {
            CrossStoreOperationState.Prepared when failed.Length == 0 =>
                "Resume the Zhinu workflow from its first incomplete step; participant writes are idempotent.",
            CrossStoreOperationState.Prepared =>
                RecoveryFor(operation, failed),
            CrossStoreOperationState.WorkspacePromoted =>
                "Do not roll back the accepted workspace. Verify its revision, then replay artifact, Cangjie, and Hetu publications in a forward recovery operation.",
            ...
            CrossStoreOperationState.ReconciliationRequired =>
                RecoveryFor(operation, failed),
            _ => "Inspect the operation receipts and Zhinu history before taking action."
        };
        return new(
            operation.Id,
            operation.State,
        ...
        IReadOnlyCollection<string> failed)
    {
        ...
        return operation.ReconciliationReason is null
            ? "Inspect Zhinu history and start a forward recovery operation."
