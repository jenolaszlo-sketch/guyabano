...
/// <summary>
/// Spec §20 Test 2 (add work after execution starts): while the implementation
/// runs, a newly discovered Cache work unit is added. The workflow mutates:
/// completed unaffected branches stay complete, the new branch executes, the
/// changed join and its test rerun, and previous artifacts remain available.
/// </summary>
public sealed class AddWorkMutationTests : IDisposable
{
    ...
    [Fact]
    public async Task Added_work_unit_executes_while_completed_branches_stay_complete()
    {
        ...
        // v1: planning, then publish the baseline cascade.
        ...
        // v2: implement both branches, join, and test.
        ...
        CallsFor(activity, "implement_cache").Should().BeEmpty();

        // Discovery: the classifier needs cache support. Publish only the new
        // planning artifacts; the existing cascade stays valid (additive).
        var cacheArtifacts = CreateStagedPayloads(withCache: true);
        var topologyV2 = await artifactCatalog.PublishAsync(
            new PublishPlanningArtifactRequest<SolutionTopology>(
                ...
                cacheArtifacts.Topology,
                Inputs: [versions.Domain],
                State: PlanningArtifactState.Valid),
            ct);
        var cacheContractKey = StagedPlanningArtifactPublisher.ContextKey(
            StagedPlanningArtifactPublisher.ContractKind, "Cache");
        var cacheComponentKey = StagedPlanningArtifactPublisher.ContextKey(
        ...
        var cacheContract = await artifactCatalog.PublishAsync(
            new PublishPlanningArtifactRequest<BoundedContextContractCatalog>(
                WorkflowId, cacheContractKey, 1,
                StagedPlanningArtifactPublisher.PlanContracts,
                cacheArtifacts.Contracts.Single(c => c.BoundedContextName == "Cache"),
        ...
        var cacheComponent = await artifactCatalog.PublishAsync(
            new PublishPlanningArtifactRequest<BoundedContextComponentManifest>(
                WorkflowId, cacheComponentKey, 1,
                StagedPlanningArtifactPublisher.PlanComponents,
                cacheArtifacts.Components.Single(c => c.BoundedContextName == "Cache"),
                Inputs: [cacheContract.Version],
                State: PlanningArtifactState.Valid),
            ct);

        // Existing artifacts remain valid; nothing was invalidated.
        foreach (var version in versions.Contracts.Values.Concat(versions.Components.Values))
        {
            (await artifactCatalog.GetAsync(WorkflowId, version, ct))!.State
                .Should().Be(PlanningArtifactState.Valid);
        }

        // Regenerate the design with the Cache branch and a three-way join.
        var extendedContracts = artifacts.ContractCatalogs
            .Concat(cacheArtifacts.Contracts.Where(c => c.BoundedContextName == "Cache"))
            .ToList();
        var extendedComponents = artifacts.ComponentManifests
            .Concat(cacheArtifacts.Components.Where(c => c.BoundedContextName == "Cache"))
            .ToList();
        var extendedArtifacts = new StagedPlanningArtifacts(
            artifacts.Domain, cacheArtifacts.Topology, extendedContracts, extendedComponents);
        ...
        extendedDesign.Graph.Steps.Select(step => step.Id).Should().BeEquivalentTo(
            "implement_cache", "implement_reporting", "implement_todos", "integrate", "test");
        await publisher.PublishExecutionDesignAsync(
            WorkflowId, extendedDesign,
            [.. extendedVersions.Contracts.Values, .. extendedVersions.Components.Values],
            cancellationToken: ct);

        // v3: add the cache branch; the changed join and test rerun.
        ...
        var v3Id = await engine.ForkAsync(
            ...
            new ForkRunOptions { TargetWorkflowVersion = "3", Actor = "test", Reason = "classifier requires cache" },
            ct);
        ...
        // Completed branches stayed complete; the new branch ran; the changed
        // join and its test reran against the extended inputs.
        ...
        // Previous artifacts remain available alongside the new ones.
        foreach (var version in versions.Contracts.Values.Concat(versions.Components.Values)
                     .Append(cacheContract.Version).Append(cacheComponent.Version))
        {
            (await artifactCatalog.GetAsync(WorkflowId, version, ct)).Should().NotBeNull();
        }
    }
