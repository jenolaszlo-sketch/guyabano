...
/// <summary>
/// Spec §20 Test 5 (failed candidate mutation): an invalid Fuwen candidate
/// fails closed — the current workflow is untouched, nothing is partially
/// applied, and the candidate plus its diagnostics remain available. A repair
/// loop then produces an admitted candidate and the mutation proceeds,
/// executing only the new node.
/// </summary>
public sealed class FailedCandidateMutationTests : IDisposable
{
    private const string WorkflowId = "plan";

    private readonly string _root = Path.Combine(
        Path.GetTempPath(),
        "guyabano-failed-candidate",
    ...
    private static string BadV3Dsl() => $$"""
        workflow plan(input: string) -> string {
          activity domain = activity "sample.stage@1#{{Digest('a')}}" (value: input;) -> string;
          activity topology = activity "sample.stage@1#{{Digest('a')}}" (value: domain;) -> string;
          activity implement = activity "sample.task@1#{{Digest('b')}}" (value: topology;) -> string;
          activity verify = activity "sample.check@1#{{new string('9', 64)}}" (value: implement;) -> string;
          return verify;
        }
        """;

    private static string GoodV3Dsl() => $$"""
        workflow plan(input: string) -> string {
          activity domain = activity "sample.stage@1#{{Digest('a')}}" (value: input;) -> string;
          activity topology = activity "sample.stage@1#{{Digest('a')}}" (value: domain;) -> string;
          activity implement = activity "sample.task@1#{{Digest('b')}}" (value: topology;) -> string;
          activity verify = activity "sample.check@1#{{Digest('d')}}" (value: implement;) -> string;
          return verify;
        }
        """;

    [Fact]
    public async Task Invalid_candidate_fails_closed_and_repair_mutates_only_the_new_node()
    {
        ...
        // v1: planning, then publish its artifacts.
        ...
        var domain = await artifactCatalog.PublishAsync(
            ...
            ct);
        var topology = await artifactCatalog.PublishAsync(
            new PublishPlanningArtifactRequest<string>(
                WorkflowId,
        ...
        // v2: implement, fork, and execute.
        var designV2 = Design(
            ("implement", "sample.task", 'b', []),
            ("verify", null, null, null));
        var authored2 = await AuthorAsync(designV2, V2Dsl(), maxAttempts: 1, ct);
        authored2.Succeeded.Should().BeTrue(string.Join("; ", authored2.Diagnostics));
        var admission2 = authored2.Admission!;
        ...
        // Candidate v3 with an invalid descriptor digest fails admission.
        var designV3 = Design(
            ("implement", "sample.task", 'b', []),
            ("verify", "sample.check", 'd', ["implement"]));
        var failed = await AuthorAsync(designV3, BadV3Dsl(), maxAttempts: 1, ct);
        failed.Succeeded.Should().BeFalse();
        failed.Admission.Should().BeNull();

        // The failed candidate and its diagnostics are preserved for review,
        // while the live workflow is untouched and nothing was applied.
        var candidate = await artifactCatalog.PublishAsync(
            new PublishPlanningArtifactRequest<FuwenCandidatePayload>(
                WorkflowId,
                new PlanningArtifactKey("fuwen-candidate", "main"),
                1,
                "author-fuwen",
                new FuwenCandidatePayload(failed.Attempts[0].Dsl, failed.Diagnostics),
                Inputs: [topology.Version],
                State: PlanningArtifactState.Candidate),
            ct);
        var stored = await artifactCatalog.ReadPayloadAsync<FuwenCandidatePayload>(
            (await artifactCatalog.GetAsync(WorkflowId, candidate.Version, ct))!, ct);
        stored.Dsl.Should().Contain("sample.check");
        stored.Diagnostics.Should().NotBeEmpty();
        (await engine.GetRunAsync(v2Id, ct))!.Status.Should().Be(WorkflowStatus.Completed);
        CallsFor(activity, "implement").Should().HaveCount(1);
        CallsFor(activity, "verify").Should().BeEmpty();

        // Repair produces an admitted candidate; the mutation then executes
        // only the new node.
        var repaired = await AuthorRepairAsync(designV3, ct);
        repaired.Succeeded.Should().BeTrue();
        repaired.Attempts.Should().HaveCount(2);
        var admission3 = repaired.Admission!;
        ...
        var v3Id = await engine.ForkAsync(
            v2Id, "plan/verify",
            new ForkRunOptions { TargetWorkflowVersion = "3", Actor = "test", Reason = "repaired candidate" },
            ct);
        await engine.ExecuteAsync(v3Id, ct);
        await engine.WaitForCompletionAsync<JsonElement>(v3Id, cancellationToken: ct);

        CallsFor(activity, "implement").Should().HaveCount(1);
        CallsFor(activity, "verify").Should().HaveCount(1);
            ...
            bindings.Add(new PlanningNodeBinding
            {
                ...
                Binding = new PlanningBinding
                {
                    ...
                    Descriptor = new DescriptorReference(
                        DescriptorKind.Activity, descriptor, "1",
                        new ContentDigest("sha256", "descriptor/v1", Digest(digest.Value))),
                },
            });
        }
